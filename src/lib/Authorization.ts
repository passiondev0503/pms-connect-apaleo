...
export async function ApaleoGenerateAccessToken(
  options: IApaleoGenerateTokenOptions
): Promise<IApaleoGenerateTokenResponse | null> {
  let { client_id, client_secret, redirect_uri } = options;
  let payload = options.code
    ? `grant_type=authorization_code&code=${options.code}&redirect_uri=${redirect_uri}`
    : `grant_type=refresh_token&refresh_token=${options.refresh}`;
